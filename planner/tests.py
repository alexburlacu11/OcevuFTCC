...
#         self.planning.displayGUI()
    ...
    def test_PLAN_nonfunc_planner_Planning_schedule_durationOfScheduling(self):   
        ...
        myList = [] 
        f = open('result_performances.txt', 'r+')
...
#             del self.planning.sequencesHistory[:]
    ...
    """Alain Klotz CADOR sequences tests"""
    
    def test_PLAN_planner_Planning_schedule_planSequencesFromCador(self):
        """
        precond: sequences in database, empty planning
        action: Test if the planning is correctly loaded with the sequences from cador
        postcond: non empty planning
        """
                       
        myList = []
        f = open('result_performances_ak.txt', 'r+')
        for i in range(0,1):       
...
#         self.planning.display()
    ...
    def test_PLAN_planner_Planning_schedule_planSequencesFromCador_OFFLINE(self):
        """
          
        TODO: THIS TEST POSES PROBLEMS FOR OTHER TESTS
          
        precond: sequences in database, empty planning
        action: Test if the planning is correctly loaded with the sequences from cador offline
        postcond: non empty planning
        """
                       
        myList = []
        f = open('result_performances_ak_offline.txt', 'r+')
        for i in range(0,1):       
            self.planning.initFromCador(self.owner1, self.quota1, True)    
            planStart = 2457126.00000001
            planEnd = 2457128.00000001
            self.planning.initFromDB(planStart, planEnd)   
            t0 = t.clock()
...
#         self.planning.display()
    ...
    def test_PLAN_planner_Planning_schedule_planningKlotz(self):
        """
        TODO: Format getKlotzSequenceIDs CF initFromCador function
        ...
        self.planning.initFromCador(self.owner1, self.quota1, False)   
        """CHANGE THIS for current planning online """
        planStart = 2457126.00000001
        planEnd = 2457128.00000001 
        self.planning.initFromDB(planStart, planEnd)
#         self.planning.display()
...
#         self.planning.display()
        ...
        listOfSeqInKlotz= self.planning.getKlotzSequenceIDs(False)
