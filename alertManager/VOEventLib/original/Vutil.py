...
def export_to_string(v, *args):
    '''
    Export the event XML to a string.  Return the string.
    '''
    outstream = StringIO()
    v.export(outstream, *args)
...
def whereWhenDict(v):
    ...
    posError: positional error in degrees.
    '''
    wwd = {}
    ww = v.get_WhereWhen()
    if not ww:
    ...
    if not pos:
        return wwd
    wwd['posError']  = pos.get_Error2Radius()
    v2 = pos.get_Value2()
    if not v2:
        return wwd
...
# properly typed values.
    ...
    def setValue(self, name, irow, value, out=sys.stdout):
        ...
        '''
        if self.colNames.has_key(name):
            icol = self.colNames[name]
        else:
            print>>out, "setTable: Unknown column name %s. Known list is %s" % (name, str(self.colNames.keys()))
            return False
